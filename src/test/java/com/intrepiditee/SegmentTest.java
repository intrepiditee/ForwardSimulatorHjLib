...
public class SegmentTest {
    ...
    @Test
    void testIntersect() {
        Segment seg = make(-10, 4, 0);

        Segment seg1 = make(-10, 5, 0);
        Segment seg2 = make(-5, 11, 0);
        Segment seg3 = make(4, 5, 0);
        Segment seg4 = make(5, 10, 0);
    ...
    void testCanMergeAndMerge() {
        Segment seg = make(1, 2, 5);

        Segment seg1 = make(2, 3, 5);
        assertTrue(canMerge(seg, seg1));
        assertEquals(make(1, 3, 5), merge(seg, seg1));

        Segment seg2 = make(3, 4, 5);
        assertFalse(canMerge(seg, seg2));

        Segment seg3 = make(2, 3, 4);
    ...
    void testSplit() {
        ...
        seg = make(4, 10, 3);
        expected = Arrays.asList(make(5, 10, 3));
        split = seg.split(Arrays.asList(indices));
        assertEquals(expected, split);

        indices = new Integer[]{9};
        expected = Arrays.asList(make(4, 9, 3));
        split = seg.split(Arrays.asList(indices));
        assertEquals(expected, split);

        indices = new Integer[]{4, 9};
        expected = Arrays.asList(make(5, 9, 3));
        split = seg.split(Arrays.asList(indices));
        assertEquals(expected, split);
