...
public class PedigreeGraphTest {
    ...
    @Test
    void test() {
        PedigreeGraph.main(new String[]{"--pedigree", "5", "0-3", "0-3", "3", "10", "3"});

        Map<Pair<Integer, Integer>, Integer> degrees = new HashMap<>();
        Scanner sc = Utils.getScannerFromGZip("cts/ibd/ForwardSimulatorHjLib/target/degree/degree_1.txt.gz");
        ...
        degrees = new HashMap<>();
        sc = Utils.getScannerFromGZip("cts/ibd/ForwardSimulatorHjLib/target/degree/degree_2.txt.gz");
        while (sc.hasNextLine()) {
            String[] ids = sc.nextLine().split("\t");
            int id1 = Integer.parseInt(ids[0]);
            int id2 = Integer.parseInt(ids[1]);
        ...
        degrees = new HashMap<>();
        sc = Utils.getScannerFromGZip("cts/ibd/ForwardSimulatorHjLib/target/degree/degree_3.txt.gz");
        while (sc.hasNextLine()) {
            String[] ids = sc.nextLine().split("\t");
            int id1 = Integer.parseInt(ids[0]);
            int id2 = Integer.parseInt(ids[1]);
